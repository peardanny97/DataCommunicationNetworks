...
def plot_gamma():
    ...
    a = stats.gamma.pdf(time_slice, a=2, scale=2)
    b = stats.gamma.pdf(time_slice, a=2, scale=1)
    c = stats.gamma.pdf(time_slice, a=4, scale=2)
    d = stats.gamma.pdf(time_slice, a=6, scale=2)

    labels = ["alpha=2, beta=2", "alpha=2, beta=1", "alpha=4, beta=2", "alpha=6, beta=2"]
    plt.plot(time_slice, a, label=labels[0])
    plt.plot(time_slice, b, label=labels[1])
    plt.plot(time_slice, c, label=labels[2])
    plt.plot(time_slice, d, label=labels[3])
    plt.legend()
    plt.show()
