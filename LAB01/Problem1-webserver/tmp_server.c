...
int respond(int sock) {
  ...
  /* make HTTP response */
  char HTTP_START[] = "HTTP/1.1 200 OK\r\n";
  ...
  printf("HTTP_CONTENT is %s length is %lu\n",HTTP_CONTENT, strlen(HTTP_CONTENT));

  char HTTP_HDR[200];
  bzero(HTTP_HDR, 200);
  sprintf(HTTP_HDR,"%s%s\r\n\r\n",HTTP_START,HTTP_CONTENT);
  printf("HTTP_HDR: %s\n", HTTP_HDR);
  ...
  /* if file is in text form use "r" */
  FILE* file;
  if ( (strcmp(extension, "html") == 0) || (strcmp(extension, "css") == 0) || (strcmp(extension, "js" ) == 0) )
    file = fopen(file_path,"r");
  /* for png use rb(binary) */
  else
  ...
  int file_size = ftell(file);  // get file size of requested file
  fseek(file, 0, SEEK_SET);  // return file ptr to start point

  char *HTTP_FILE = (char *)malloc(file_size * sizeof(char));
  fread(HTTP_FILE, 1, file_size, file);

  sendMessage(sock, HTTP_FILE);
  sendMessage(sock, "\r\n\r\n");
